...
public class Willy : MonoBehaviour, IHittable
{
    ...
    private void RemoveBuff()
    {
        if (components.Category == Category.Player)
        {
            foreach (var t in battlefield.playerUnits)
            {
                if (!t.IsDeath())
                {
                    t.RemoveBuff(BuffType.SkillResistant, components.Stats.FinalSkill1Values);
                }
            }
        }
        else
    ...
    private void SetBuff()
    {
        if (components.Category == Category.Player)
        {
            foreach (var t in battlefield.playerUnits)
            {
                if (!t.IsDeath())
                {
                    t.SetBuff(BuffType.SkillResistant, components.Stats.FinalSkill1Values);
                }
            }
        }
        else
